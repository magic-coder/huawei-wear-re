...
public class SlidingPaneLayout extends ViewGroup {
    ...
    class SavedState extends AbsSavedState {
        public static final Creator<SavedState> CREATOR = ParcelableCompat.newCreator(new C01771());
        boolean isOpen;

        final class C01771 implements ParcelableCompatCreatorCallbacks<SavedState> {
            C01771() {
            }
        ...
        while (i7 < childCount) {
            ...
            if (childAt.getVisibility() == 8) {
                ...
                if (layoutParams.slideable) {
                    ...
                    i5 = (int) ((DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - this.mSlideOffset) * ((float) this.mParallaxBy));
                    i10 = paddingRight;
                }
                if (isLayoutRtlSupport) {
                    i9 = (i6 - i10) + i5;
    ...
    private boolean openPane(View view, int i) {
        if (!this.mFirstLayout && !smoothSlideTo(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, i)) {
            return false;
        }
            ...
            if ((isLayoutRtlSupport ? layoutParams.rightMargin : layoutParams.leftMargin) <= 0) {
                ...
                for (i = 0; i < childCount; i++) {
                    ...
                    if (childAt == this.mSlideableView) {
                        i2 = (int) ((DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - this.mParallaxOffset) * ((float) this.mParallaxBy));
                        this.mParallaxOffset = f;
                        i2 -= (int) ((DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - f) * ((float) this.mParallaxBy));
                        if (isLayoutRtlSupport) {
                            i2 = -i2;
                        }
                        childAt.offsetLeftAndRight(i2);
                        if (obj == null) {
                            dimChildView(childAt, isLayoutRtlSupport ? this.mParallaxOffset - DefaultRetryPolicy.DEFAULT_BACKOFF_MULT : DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - this.mParallaxOffset, this.mCoveredFadeColor);
                        }
                    }
                }
            }
