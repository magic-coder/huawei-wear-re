...
public abstract class SharedElementCallback {
    ...
    public Parcelable onCaptureSharedElementSnapshot(View view, Matrix matrix, RectF rectF) {
        ...
        float min = Math.min(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, ((float) MAX_IMAGE_SIZE) / ((float) (round * round2)));
    ...
    private static Bitmap createDrawableBitmap(Drawable drawable) {
        ...
        float min = Math.min(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT, ((float) MAX_IMAGE_SIZE) / ((float) (intrinsicWidth * intrinsicHeight)));
        if ((drawable instanceof BitmapDrawable) && min == DefaultRetryPolicy.DEFAULT_BACKOFF_MULT) {
