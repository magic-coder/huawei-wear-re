...
public final class ColorUtils {
    ...
    public static void RGBToHSL(@IntRange(from = 0, to = 255) int i, @IntRange(from = 0, to = 255) int i2, @IntRange(from = 0, to = 255) int i3, @NonNull float[] fArr) {
        ...
        if (max == min) {
            ...
            f2 = f4 / (DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - Math.abs((2.0f * f5) - DefaultRetryPolicy.DEFAULT_BACKOFF_MULT));
        }
        f = (f * 60.0f) % 360.0f;
        if (f < 0.0f) {
            f += 360.0f;
        ...
        fArr[1] = constrain(f2, 0.0f, (float) DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
        fArr[2] = constrain(f5, 0.0f, (float) DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
    }
    ...
    public static int HSLToColor(@NonNull float[] fArr) {
        ...
        float f2 = fArr[1];
        float f3 = fArr[2];
        float abs = (DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - Math.abs((2.0f * f3) - DefaultRetryPolicy.DEFAULT_BACKOFF_MULT)) * f2;
        float f4 = f3 - (0.5f * abs);
        float abs2 = abs * (DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - Math.abs(((f / 60.0f) % 2.0f) - DefaultRetryPolicy.DEFAULT_BACKOFF_MULT));
        switch (((int) f) / 60) {
            case 0:
    ...
    public static void RGBToXYZ(@IntRange(from = 0, to = 255) int i, @IntRange(from = 0, to = 255) int i2, @IntRange(from = 0, to = 255) int i3, @NonNull double[] dArr) {
        ...
        dArr[1] = XYZ_WHITE_REFERENCE_Y * (((0.2126d * pow) + (0.7152d * pow2)) + (0.0722d * d));
    ...
    public static void LABToXYZ(@FloatRange(from = 0.0d, to = 100.0d) double d, @FloatRange(from = -128.0d, to = 127.0d) double d2, @FloatRange(from = -128.0d, to = 127.0d) double d3, @NonNull double[] dArr) {
        ...
        dArr[1] = pow * XYZ_WHITE_REFERENCE_Y;
    ...
    public static int blendARGB(@ColorInt int i, @ColorInt int i2, @FloatRange(from = 0.0d, to = 1.0d) float f) {
        float f2 = DefaultRetryPolicy.DEFAULT_BACKOFF_MULT - f;
    ...
    public static void blendLAB(@NonNull double[] dArr, @NonNull double[] dArr2, @FloatRange(from = 0.0d, to = 1.0d) double d, @NonNull double[] dArr3) {
        ...
        double d2 = WeightedLatLng.DEFAULT_INTENSITY - d;
    ...
    static float circularInterpolate(float f, float f2, float f3) {
        if (Math.abs(f2 - f) > BitmapDescriptorFactory.HUE_CYAN) {
            if (f2 > f) {
                f += 360.0f;
            } else {
                f2 += 360.0f;
