...
class MessageThreadUtil<T> implements ThreadUtil<T> {
            ...
            if (this.mRoot != null) {
                ...
                while (access$200 != null) {
                    ...
                    if (access$200.what == i) {
                        syncQueueItem.next = access$2002;
                        access$200.recycle();
                    } else {
                        syncQueueItem = access$200;
                    }
    ...
    public MainThreadCallback<T> getMainThreadProxy(final MainThreadCallback<T> mainThreadCallback) {
        return new MainThreadCallback<T>() {
            ...
            private Runnable mMainThreadRunnable = new C02171();
            ...
            class C02171 implements Runnable {
                ...
                public void run() {
                    SyncQueueItem next = C02181.this.mQueue.next();
                    while (next != null) {
                        switch (next.what) {
                            case 1:
    ...
    public BackgroundCallback<T> getBackgroundProxy(final BackgroundCallback<T> backgroundCallback) {
        return new BackgroundCallback<T>() {
            ...
            private Runnable mBackgroundRunnable = new C02191();
            ...
            class C02191 implements Runnable {
                ...
                public void run() {
                    while (true) {
                        SyncQueueItem next = C02202.this.mQueue.next();
                        if (next != null) {
                            switch (next.what) {
                                case 1:
                                    C02202.this.mQueue.removeMessages(1);
                                    backgroundCallback.refresh(next.arg1);
                                    break;
                                case 2:
                                    C02202.this.mQueue.removeMessages(2);
                                    C02202.this.mQueue.removeMessages(3);
                        ...
                        C02202.this.mBackgroundRunning.set(false);
                        return;
                    }
                }
            }
