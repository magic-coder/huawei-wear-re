...
public abstract class AbstractVerifier implements X509HostnameVerifier {
    ...
    public final void verify(String str, String[] strArr, String[] strArr2, boolean z) throws SSLException {
        ...
        while (it.hasNext()) {
            ...
            i = (split.length < 3 || !split[0].endsWith("*") || !acceptableCountryWildcard(toLowerCase2) || isIPAddress(str)) ? 0 : 1;
            if (i != 0) {
                if (split[0].length() > 1) {
                    String substring = split[0].substring(0, split.length - 2);
                    String substring2 = toLowerCase2.substring(split[0].length());
                    String substring3 = toLowerCase.substring(substring.length());
                    if (toLowerCase.startsWith(substring) && substring3.endsWith(substring2)) {
                        z2 = true;
                    } else {
                        z2 = false;
                    }
