...
public final class C6325r {
    public static String f22035a = "ABCDEFGHIJKLMNOPQRSTUVWXYZ234567";
    public static int[] f22036b = new int[]{0, 49345, 49537, 320, 49921, 960, 640, 49729, 50689, 1728, WBConstants.SDK_NEW_PAY_VERSION, 51009, 1280, 50625, 50305, 1088, 52225, 3264, 3456, 52545, 3840, 53185, 52865, 3648, 2560, 51905, 52097, 2880, 51457, 2496, 2176, 51265, 55297, 6336, 6528, 55617, 6912, 56257, 55937, 6720, 7680, 57025, 57217, 8000, 56577, 7616, 7296, 56385, 5120, 54465, 54657, 5440, 55041, 6080, 5760, 54849, 53761, 4800, 4992, 54081, 4352, 53697, 53377, 4160, 61441, 12480, 12672, 61761, 13056, 62401, 62081, 12864, 13824, 63169, 63361, 14144, 62721, 13760, 13440, 62529, 15360, 64705, 64897, 15680, 65281, 16320, 16000, 65089, 64001, 15040, 15232, 64321, 14592, 63937, 63617, 14400, 10240, 59585, 59777, 10560, 60161, 11200, 10880, 59969, 60929, 11968, 12160, 61249, 11520, 60865, 60545, 11328, 58369, 9408, 9600, 58689, 9984, 59329, 59009, 9792, 8704, 58049, 58241, 9024, 57601, 8640, 8320, 57409, 40961, 24768, 24960, 41281, 25344, 41921, 41601, 25152, 26112, 42689, 42881, 26432, 42241, 26048, 25728, 42049, 27648, 44225, 44417, 27968, 44801, 28608, 28288, 44609, 43521, 27328, 27520, 43841, 26880, 43457, 43137, 26688, 30720, 47297, 47489, 31040, 47873, 31680, 31360, 47681, 48641, 32448, 32640, 48961, 32000, 48577, 48257, 31808, 46081, 29888, 30080, 46401, 30464, 47041, 46721, 30272, 29184, 45761, 45953, 29504, 45313, 29120, FitnessComm.GTM_800_OFFSET_IN_SECOND, 45121, 20480, 37057, 37249, 20800, 37633, 21440, 21120, 37441, 38401, 22208, 22400, 38721, 21760, 38337, 38017, 21568, 39937, 23744, 23936, 40257, 24320, 40897, 40577, 24128, 23040, 39617, 39809, 23360, 39169, 22976, 22656, 38977, 34817, 18624, 18816, 35137, 19200, 35777, 35457, 19008, 19968, 36545, 36737, 20288, 36097, 19904, 19584, 35905, 17408, 33985, 34177, 17728, 34561, 18368, 18048, 34369, 33281, 17088, 17280, 33601, 16640, 33217, 32897, 16448};

    static {
        int[] iArr = new int[]{93629, 99879, 79843, 75029, 59699, 55667, 46867, 38039};
    }

    public static double m28993a(double d, int i) {
        double d2 = 0.0d;
        try {
            if (!Double.isNaN(d)) {
    ...
    private static String m28994a(int i, int i2, int i3, int i4, int i5, int i6, int i7) {
    ...
    public static String m28995a(C6318j c6318j, List<NeighboringCellInfo> list) {
        int i = 0;
        if (c6318j == null) {
            return "[]";
        }
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("[");
        int i2 = c6318j.f22013b;
        int i3 = c6318j.f22014c;
        if (C6325r.m28999a(c6318j.f22012a, i2, i3, c6318j.f22015d, c6318j.f22016e)) {
            stringBuilder.append(C6325r.m28994a(i2, i3, c6318j.f22015d, c6318j.f22016e, c6318j.f22017f, c6318j.f22018g, c6318j.f22019h));
            i = 1;
        }
        if (list != null) {
            try {
                ...
                for (NeighboringCellInfo neighboringCellInfo : list) {
                    ...
                    if (C6325r.m28999a(c6318j.f22012a, i2, i3, parseInt, neighboringCellInfo.getCid())) {
                        if (i > 0) {
                            stringBuilder.append(",");
                        }
                        stringBuilder.append(C6325r.m28994a(i2, i3, parseInt, neighboringCellInfo.getCid(), (neighboringCellInfo.getRssi() << 1) - 113, Integer.MAX_VALUE, Integer.MAX_VALUE));
                        parseInt = i + 1;
                    } else {
                        parseInt = i;
                    }
    ...
    public static String m28996a(C6320l c6320l) {
        if (c6320l.m28989b() == null) {
            return "{}";
        }
        Location b = c6320l.m28989b();
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("{");
    ...
    public static String m28997a(C6323o c6323o) {
        if (c6323o == null || c6323o.m28991a() == null) {
            return "[]";
        }
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("[");
        if (c6323o.m28991a() == null || c6323o.m28991a().size() <= 0) {
            stringBuilder.append("]");
            return stringBuilder.toString();
        }
        List<ScanResult> a = c6323o.m28991a();
        int i = 0;
    ...
    public static String m28998a(String str, String str2, String str3, String str4, boolean z) {
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("{");
    ...
    public static boolean m28999a(int i, int i2, int i3, int i4, int i5) {
    ...
    public static boolean m29000a(String str) {
        try {
