...
public class MapTilsCacheAndResManagerImpl extends MapTilsCacheAndResManager {
    ...
    public void saveFile(String str, int i, int i2, byte[] bArr) {
        if (bArr != null && bArr.length > 0) {
            ...
            FileUtil.writeDatasToFile(this.mMapExtResPath + (str + HwAccountConstants.SPLIIT_UNDERLINE + i + HwAccountConstants.SPLIIT_UNDERLINE + i2 + ".data"), bArr);
        }
    }

    private String getFilePreName(String str) {
        String[] split = str.split(HwAccountConstants.SPLIIT_UNDERLINE);
        return split[0] + HwAccountConstants.SPLIIT_UNDERLINE + split[1] + HwAccountConstants.SPLIIT_UNDERLINE + split[2];
    }
    ...
    private void setRetFile(RetStyleIconsFile retStyleIconsFile, String str) {
        if (retStyleIconsFile != null) {
            ...
            retStyleIconsFile.name = split[0] + HwAccountConstants.SPLIIT_UNDERLINE + split[1];
