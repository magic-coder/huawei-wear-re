...
public abstract class Request<T> implements Comparable<Request<T>> {
    ...
    void finish(final String str) {
        ...
        if (SystemClock.elapsedRealtime() - this.mRequestBirthTime >= SLOW_REQUEST_THRESHOLD_MS) {
            VolleyLog.m123d("%d ms: %s", Long.valueOf(SystemClock.elapsedRealtime() - this.mRequestBirthTime), toString());
        }
    }
    ...
    public String toString() {
        return (this.mCanceled ? "[X] " : "[ ] ") + getUrl() + HwAccountConstants.BLANK + ("0x" + Integer.toHexString(getTrafficStatsTag())) + HwAccountConstants.BLANK + getPriority() + HwAccountConstants.BLANK + this.mSequence;
    }
