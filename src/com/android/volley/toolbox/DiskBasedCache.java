...
public class DiskBasedCache implements Cache {
    ...
    public synchronized void clear() {
        synchronized (this) {
            ...
            this.mEntries.clear();
            this.mTotalSize = 0;
            VolleyLog.m123d("Cache cleared.", new Object[0]);
        }
    }
        ...
        if (cacheHeader == null) {
            ...
            try {
                ...
                try {
                    ...
                    e = e4;
                    try {
                        VolleyLog.m123d("%s: %s", fileForKey.getAbsolutePath(), e.toString());
                        remove(str);
                        if (countingInputStream != null) {
                            try {
                                countingInputStream.close();
                    ...
                    e2 = e7;
                    VolleyLog.m123d("%s: %s", fileForKey.getAbsolutePath(), e2.toString());
                    remove(str);
                    if (countingInputStream != null) {
                        try {
                            countingInputStream.close();
    ...
    public synchronized void initialize() {
        ...
        if (this.mRootDirectory.exists()) {
            ...
            VolleyLog.m124e("Unable to create cache dir %s", this.mRootDirectory.getAbsolutePath());
        }
        return;
    ...
    public synchronized void put(String str, Entry entry) {
        ...
        try {
            ...
            if (cacheHeader.writeHeader(bufferedOutputStream)) {
                ...
                bufferedOutputStream.close();
                VolleyLog.m123d("Failed to write header for %s", fileForKey.getAbsolutePath());
                throw new IOException();
            }
        } catch (IOException e) {
            if (!fileForKey.delete()) {
                VolleyLog.m123d("Could not clean up file %s", fileForKey.getAbsolutePath());
            }
        }
    }
    ...
    public synchronized void remove(String str) {
        ...
        removeEntry(str);
        if (!delete) {
            VolleyLog.m123d("Could not delete cache entry for key=%s, filename=%s", str, getFilenameForKey(str));
        }
    }
    ...
    private void pruneIfNeeded(int i) {
        if (this.mTotalSize + ((long) i) >= ((long) this.mMaxCacheSizeInBytes)) {
            int i2;
            if (VolleyLog.DEBUG) {
                VolleyLog.m126v("Pruning old cache entries.", new Object[0]);
            }
            ...
            while (it.hasNext()) {
                ...
                if (getFileForKey(cacheHeader.key).delete()) {
                    ...
                    VolleyLog.m123d("Could not delete cache entry for key=%s, filename=%s", cacheHeader.key, getFilenameForKey(cacheHeader.key));
                }
                it.remove();
                i2 = i3 + 1;
            ...
            i2 = i3;
            if (VolleyLog.DEBUG) {
                VolleyLog.m126v("pruned %d files, %d bytes, %d ms", Integer.valueOf(i2), Long.valueOf(this.mTotalSize - j), Long.valueOf(SystemClock.elapsedRealtime() - elapsedRealtime));
            }
        }
    }
