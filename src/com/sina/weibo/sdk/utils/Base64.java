...
public final class Base64 {
    ...
    static {
        ...
        codes[43] = TagName.CARD_BUSINESS_ORDER_STATUS;
        codes[47] = TagName.CARD_APP_ACTIVATION_STATUS;
    }

    public static byte[] decode(byte[] bArr) {
        int i = 0;
        int length = ((bArr.length + 3) / 4) * 3;
        if (bArr.length > 0 && bArr[bArr.length - 1] == TagName.CARD_APP_VERSION) {
            length--;
        }
        if (bArr.length > 1 && bArr[bArr.length - 2] == TagName.CARD_APP_VERSION) {
            length--;
        }
