...
public class ShareRequestParam extends BrowserRequestParamBase {
    ...
    private void handleSharedMessage(Bundle bundle) {
        ...
        if (weiboMultiMessage.mediaObject instanceof WebpageObject) {
            stringBuilder.append(HwAccountConstants.BLANK).append(((WebpageObject) weiboMultiMessage.mediaObject).actionUrl);
        }
        if (weiboMultiMessage.mediaObject instanceof MusicObject) {
            stringBuilder.append(HwAccountConstants.BLANK).append(((MusicObject) weiboMultiMessage.mediaObject).actionUrl);
        }
        if (weiboMultiMessage.mediaObject instanceof VideoObject) {
            stringBuilder.append(HwAccountConstants.BLANK).append(((VideoObject) weiboMultiMessage.mediaObject).actionUrl);
        }
        if (weiboMultiMessage.mediaObject instanceof VoiceObject) {
            stringBuilder.append(HwAccountConstants.BLANK).append(((VoiceObject) weiboMultiMessage.mediaObject).actionUrl);
        }
