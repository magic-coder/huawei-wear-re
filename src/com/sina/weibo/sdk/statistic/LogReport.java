...
import cn.com.fmsh.communication.message.constants.Constants.XMLNode.XMLTag;
...
class LogReport {
    ...
    public LogReport(Context context) {
        try {
            ...
            LogUtil.m28662e(WBAgent.TAG, e.toString());
        }
        initCommonParams();
    }
        ...
        synchronized (LogReport.class) {
            ...
            if (isNetworkConnected(context)) {
                ...
                if (validUploadLogs == null) {
                    LogUtil.m28663i(WBAgent.TAG, "applogs is null");
                } else {
                    List<JSONArray> arrayList = new ArrayList();
                    checkAid(context);
                    for (JSONArray jSONArray : validUploadLogs) {
                        HttpResponse requestHttpExecute = requestHttpExecute(mBaseUrl, HttpPost.METHOD_NAME, mParams, jSONArray);
                        if (requestHttpExecute != null && requestHttpExecute.getStatusLine().getStatusCode() == 200) {
                            ...
                            LogUtil.m28662e(WBAgent.TAG, "upload applogs error");
                        }
                    }
                    ...
                    if (arrayList.size() > 0) {
                        for (JSONArray jSONArray2 : arrayList) {
                            ...
                            LogUtil.m28661d(WBAgent.TAG, "save failed_log");
                        }
                    }
                }
            } else {
                LogUtil.m28663i(WBAgent.TAG, "network is not connected");
    ...
    private static HttpResponse requestHttpExecute(String str, String str2, JSONObject jSONObject, JSONArray jSONArray) {
        ...
        try {
            ...
            try {
                jSONObject.put(HciConfigInfo.HCI_DATA_TYPE_TRANSCTION_TIME, System.currentTimeMillis() / 1000);
                jSONObject.put(XMLTag.TAG_LENGTH, jSONArray.length());
                jSONObject.put("sign", getSign(jSONObject.getString("aid"), jSONObject.getString(LogBuilder.KEY_APPKEY), jSONObject.getLong(HciConfigInfo.HCI_DATA_TYPE_TRANSCTION_TIME)));
                jSONObject.put("content", jSONArray);
                LogUtil.m28661d(WBAgent.TAG, "post content--- " + jSONObject.toString());
            } catch (JSONException e11) {
                e11.printStackTrace();
            }
            if (str2.equals(HttpGet.METHOD_NAME)) {
                httpGet = new HttpGet(new StringBuilder(String.valueOf(str)).append(LocationInfo.NA).append(jSONObject.toString()).toString());
                byteArrayOutputStream = null;
            } else if (!str2.equals(HttpPost.METHOD_NAME)) {
                httpGet = null;
                byteArrayOutputStream = null;
            } else if (TextUtils.isEmpty(mAppkey)) {
                LogUtil.m28662e(WBAgent.TAG, "unexpected null AppKey");
                if (null != null) {
                    try {
                        byteArrayOutputStream.close();
                ...
                httpGet = getNewHttpPost(new StringBuilder(String.valueOf(str)).append(LocationInfo.NA).append("source=").append(mAppkey).toString(), jSONObject);
            ...
            try {
                ...
                LogUtil.m28663i(WBAgent.TAG, "status code = " + httpResponse.getStatusLine().getStatusCode());
                if (byteArrayOutputStream != null) {
                    try {
                        byteArrayOutputStream.close();
    ...
    private static boolean isNetworkConnected(Context context) {
        if (context == null) {
            LogUtil.m28662e(WBAgent.TAG, "unexpected null context in isNetworkConnected");
            return false;
