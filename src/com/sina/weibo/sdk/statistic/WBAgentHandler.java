...
class WBAgentHandler {
    ...
    private WBAgentHandler() {
        ...
        mPages = new HashMap();
        LogUtil.m28663i(WBAgent.TAG, "init handler");
    }
    ...
    public void onPageStart(String str) {
        if (!StatisticConfig.ACTIVITY_DURATION_OPEN) {
            ...
            LogUtil.m28661d(WBAgent.TAG, new StringBuilder(String.valueOf(str)).append(", ").append(pageLog.getStartTime() / 1000).toString());
        }
    }
    ...
    public void onPageEnd(String str) {
        if (!StatisticConfig.ACTIVITY_DURATION_OPEN) {
            if (mPages.containsKey(str)) {
                ...
                LogUtil.m28661d(WBAgent.TAG, new StringBuilder(String.valueOf(str)).append(", ").append(pageLog.getStartTime() / 1000).append(", ").append(pageLog.getDuration() / 1000).toString());
            } else {
                LogUtil.m28662e(WBAgent.TAG, "please call onPageStart before onPageEnd");
            }
    ...
    public void onResume(Context context) {
        ...
        LogUtil.m28661d(WBAgent.TAG, new StringBuilder(String.valueOf(name)).append(", ").append(currentTimeMillis / 1000).toString());
    }

    public void onPause(Context context) {
        long currentTimeMillis = System.currentTimeMillis();
        String name = context.getClass().getName();
        LogUtil.m28663i(WBAgent.TAG, "update last page endtime:" + (currentTimeMillis / 1000));
        ...
        if (StatisticConfig.ACTIVITY_DURATION_OPEN) {
            if (mPages.containsKey(name)) {
                ...
                LogUtil.m28661d(WBAgent.TAG, new StringBuilder(String.valueOf(name)).append(", ").append(pageLog.getStartTime() / 1000).append(", ").append(pageLog.getDuration() / 1000).toString());
            } else {
                LogUtil.m28662e(WBAgent.TAG, "please call onResume before onPause");
            }
            if (mActivePages.size() >= MAX_CACHE_SIZE) {
                saveActivePages(mActivePages);
                mActivePages.clear();
    ...
    public void onEvent(String str, String str2, Map<String, String> map) {
        ...
        if (map == null) {
            LogUtil.m28661d(WBAgent.TAG, "event--- page:" + str + " ,event name:" + str2);
        } else {
            LogUtil.m28661d(WBAgent.TAG, "event--- page:" + str + " ,event name:" + str2 + " ,extend:" + map.toString());
        }
        if (mActivePages.size() >= MAX_CACHE_SIZE) {
            saveActivePages(mActivePages);
            mActivePages.clear();
    ...
    private boolean isBackground(Context context) {
        for (RunningAppProcessInfo runningAppProcessInfo : ((ActivityManager) context.getSystemService("activity")).getRunningAppProcesses()) {
            if (runningAppProcessInfo.processName.equals(context.getPackageName())) {
                if (runningAppProcessInfo.importance == HttpStatus.SC_BAD_REQUEST) {
                    LogUtil.m28663i(WBAgent.TAG, "后台:" + runningAppProcessInfo.processName);
                    return true;
                }
                LogUtil.m28663i(WBAgent.TAG, "前台:" + runningAppProcessInfo.processName);
                return false;
            }
        }
        return false;
    ...
    public void onKillProcess() {
        LogUtil.m28663i(WBAgent.TAG, "save applogs and close timer and shutdown thread executor");
        saveActivePages(mActivePages);
        mInstance = null;
        closeTimer();
        ...
        if (PageLog.isNewSession(context, j)) {
            ...
            synchronized (mActivePages) {
                if (pageLog.getEndTime() > 0) {
                    ...
                    LogUtil.m28661d(WBAgent.TAG, "is a new install");
                }
                mActivePages.add(pageLog2);
            }
            LogUtil.m28661d(WBAgent.TAG, "last session--- starttime:" + pageLog.getStartTime() + " ,endtime:" + pageLog.getEndTime());
            LogUtil.m28661d(WBAgent.TAG, "is a new session--- starttime:" + pageLog2.getStartTime());
            return;
        }
        LogUtil.m28663i(WBAgent.TAG, "is not a new session");
    }
    ...
    private Timer timerTask(final Context context, long j, long j2) {
        ...
        TimerTask c62263 = new TimerTask() {
            public void run() {
                LogReport.uploadAppLogs(context, WBAgentHandler.this.getLogsInMemory());
            }
        };
        if (j2 == 0) {
            timer.schedule(c62263, j);
        } else {
            timer.schedule(c62263, j, j2);
        }
        return timer;
    }
