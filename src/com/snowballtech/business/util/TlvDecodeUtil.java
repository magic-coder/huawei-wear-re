...
public class TlvDecodeUtil {
        ...
        if (list != null && str.substring(0, str.length()).length() > 0) {
            ...
            if ((Integer.valueOf(strArr[0], 16).intValue() & 32) != 32) {
                if ((Integer.parseInt(strArr[0], 16) & 31) != 31) {
                    ...
                    list.add(new Tlv(strArr[0], stringBuilder.toString().length(), stringBuilder.toString()));
                    if (i > generateLength[1]) {
                        stringBuilder2 = new StringBuilder();
                        for (i2 = generateLength[1]; i2 < strArr.length; i2++) {
                            stringBuilder2.append(strArr[i2]);
                        }
                        decodeTLVForFull(stringBuilder2.toString(), strArr.length - generateLength[1], list);
                        return;
                    }
                    return;
                }
                ...
                list.add(new Tlv(strArr[0] + strArr[1], stringBuilder.toString().length(), stringBuilder.toString()));
                if (i > generateLength[1]) {
                    stringBuilder2 = new StringBuilder();
                    for (i2 = generateLength[1]; i2 < strArr.length; i2++) {
                        stringBuilder2.append(strArr[i2]);
    ...
    private static int[] retriveIndexOfLen(String[] strArr, int i) {
        ...
        if (strArr != null && strArr.length > 0) {
            ...
            if (isDirectValue(strArr[i])) {
                ...
                binaryToDecimal = (i + 1) + binaryToDecimal >= strArr.length ? strArr.length : binaryToDecimal + (i + 1);
    ...
    private static boolean isDirectValue(String str) {
        ...
        if (ValueUtil.toBin(Integer.valueOf(str, 16).intValue(), 8, "0").substring(0, 1).equals("1")) {
            return false;
        }
        return true;
    }
