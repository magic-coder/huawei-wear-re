...
public final class LatLng implements Parcelable, Cloneable {
    ...
    private static DecimalFormat f12055a = new DecimalFormat("0.000000", new DecimalFormatSymbols(Locale.US));
    ...
    public LatLng(double d, double d2, boolean z) {
        if (z) {
            if (-180.0d > d2 || d2 >= 180.0d) {
                this.longitude = m16458a(((((d2 - 180.0d) % 360.0d) + 360.0d) % 360.0d) - 180.0d);
            } else {
                this.longitude = m16458a(d2);
            }
            if (d < -90.0d || d > 90.0d) {
                try {
            ...
            this.latitude = m16458a(Math.max(-90.0d, Math.min(90.0d, d)));
            return;
        }
        this.latitude = d;
        this.longitude = d2;
    ...
    private static double m16458a(double d) {
        return Double.parseDouble(f12055a.format(d));
    }
