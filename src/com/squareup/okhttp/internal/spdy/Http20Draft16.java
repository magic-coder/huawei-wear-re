...
public final class Http20Draft16 implements Variant {
    ...
    final class ContinuationSource implements Source {
        ...
        private void readContinuationHeader() throws IOException {
            int i = this.streamId;
            int access$300 = Http20Draft16.readMedium(this.source);
            this.left = access$300;
            this.length = access$300;
            byte readByte = (byte) (this.source.readByte() & 255);
            this.flags = (byte) (this.source.readByte() & 255);
            if (Http20Draft16.logger.isLoggable(Level.FINE)) {
                Http20Draft16.logger.fine(FrameLogger.formatHeader(true, this.streamId, this.length, readByte, this.flags));
            }
            this.streamId = this.source.readInt() & Integer.MAX_VALUE;
            if (readByte != (byte) 9) {
                throw Http20Draft16.ioException("%s != TYPE_CONTINUATION", Byte.valueOf(readByte));
    ...
    final class Reader implements FrameReader {
        ...
        public void readConnectionPreface() throws IOException {
            if (!this.client) {
                ByteString readByteString = this.source.readByteString((long) Http20Draft16.CONNECTION_PREFACE.size());
                if (Http20Draft16.logger.isLoggable(Level.FINE)) {
                    Http20Draft16.logger.fine(String.format("<< CONNECTION %s", new Object[]{readByteString.hex()}));
                }
                if (!Http20Draft16.CONNECTION_PREFACE.equals(readByteString)) {
                    throw Http20Draft16.ioException("Expected a connection header but was %s", readByteString.utf8());
                }
            }
        }
        ...
        public boolean nextFrame(Handler handler) throws IOException {
            try {
                ...
                if (access$300 < 0 || access$300 > 16384) {
                    throw Http20Draft16.ioException("FRAME_SIZE_ERROR: %s", Integer.valueOf(access$300));
                }
                byte readByte = (byte) (this.source.readByte() & 255);
                byte readByte2 = (byte) (this.source.readByte() & 255);
                int readInt = this.source.readInt() & Integer.MAX_VALUE;
                if (Http20Draft16.logger.isLoggable(Level.FINE)) {
                    Http20Draft16.logger.fine(FrameLogger.formatHeader(true, readInt, access$300, readByte, readByte2));
                }
                switch (readByte) {
                    case (byte) 0:
        ...
        private void readHeaders(Handler handler, int i, byte b, int i2) throws IOException {
            ...
            handler.headers(false, z, i2, -1, readHeaderBlock(Http20Draft16.lengthWithoutPadding(i, b, readByte), readByte, b, i2), HeadersMode.HTTP_20_HEADERS);
        }
        ...
        private void readData(Handler handler, int i, byte b, int i2) throws IOException {
            ...
            handler.data(z, i2, this.source, Http20Draft16.lengthWithoutPadding(i, b, s2));
            this.source.skip((long) s2);
        }

        private void readPriority(Handler handler, int i, byte b, int i2) throws IOException {
            if (i != 5) {
                throw Http20Draft16.ioException("TYPE_PRIORITY length: %d != 5", Integer.valueOf(i));
            } else if (i2 == 0) {
        ...
        private void readRstStream(Handler handler, int i, byte b, int i2) throws IOException {
            if (i != 4) {
                throw Http20Draft16.ioException("TYPE_RST_STREAM length: %d != 4", Integer.valueOf(i));
            } else if (i2 == 0) {
                ...
                if (fromHttp2 == null) {
                    throw Http20Draft16.ioException("TYPE_RST_STREAM unexpected error code: %d", Integer.valueOf(r0));
                } else {
        ...
        private void readSettings(Handler handler, int i, byte b, int i2) throws IOException {
            if (i2 != 0) {
                ...
                throw Http20Draft16.ioException("TYPE_SETTINGS length %% 6 != 0: %s", Integer.valueOf(i));
            } else {
                ...
                for (int i3 = 0; i3 < i; i3 += 6) {
                    ...
                    switch (readShort) {
                        ...
                        case 5:
                            ...
                            throw Http20Draft16.ioException("PROTOCOL_ERROR SETTINGS_MAX_FRAME_SIZE: %s", Integer.valueOf(readInt));
                        default:
                            throw Http20Draft16.ioException("PROTOCOL_ERROR invalid settings id: %s", Short.valueOf(readShort));
                    }
        ...
        private void readPushPromise(Handler handler, int i, byte b, int i2) throws IOException {
            ...
            handler.pushPromise(i2, this.source.readInt() & Integer.MAX_VALUE, readHeaderBlock(Http20Draft16.lengthWithoutPadding(i - 4, b, s), s, b, i2));
        }

        private void readPing(Handler handler, int i, byte b, int i2) throws IOException {
            boolean z = true;
            if (i != 8) {
                throw Http20Draft16.ioException("TYPE_PING length != 8: %s", Integer.valueOf(i));
            } else if (i2 != 0) {
        ...
        private void readGoAway(Handler handler, int i, byte b, int i2) throws IOException {
            if (i < 8) {
                throw Http20Draft16.ioException("TYPE_GOAWAY length < 8: %s", Integer.valueOf(i));
            } else if (i2 != 0) {
                ...
                ErrorCode fromHttp2 = ErrorCode.fromHttp2(this.source.readInt());
                if (fromHttp2 == null) {
                    throw Http20Draft16.ioException("TYPE_GOAWAY unexpected error code: %d", Integer.valueOf(r0));
                }
        ...
        private void readWindowUpdate(Handler handler, int i, byte b, int i2) throws IOException {
            if (i != 4) {
                throw Http20Draft16.ioException("TYPE_WINDOW_UPDATE length !=4: %s", Integer.valueOf(i));
            }
            long readInt = ((long) this.source.readInt()) & 2147483647L;
            if (readInt == 0) {
                throw Http20Draft16.ioException("windowSizeIncrement was 0", Long.valueOf(readInt));
            } else {
    ...
    final class Writer implements FrameWriter {
        ...
        public synchronized void connectionPreface() throws IOException {
            if (this.closed) {
                ...
                if (Http20Draft16.logger.isLoggable(Level.FINE)) {
                    Http20Draft16.logger.fine(String.format(">> CONNECTION %s", new Object[]{Http20Draft16.CONNECTION_PREFACE.hex()}));
                }
        ...
        public synchronized void windowUpdate(int i, long j) throws IOException {
            if (this.closed) {
                ...
                throw Http20Draft16.illegalArgument("windowSizeIncrement == 0 || windowSizeIncrement > 0x7fffffffL: %s", Long.valueOf(j));
            } else {
        ...
        void frameHeader(int i, int i2, byte b, byte b2) throws IOException {
            if (Http20Draft16.logger.isLoggable(Level.FINE)) {
                Http20Draft16.logger.fine(FrameLogger.formatHeader(false, i, i2, b, b2));
            }
            if (i2 > this.maxFrameSize) {
                throw Http20Draft16.illegalArgument("FRAME_SIZE_ERROR length > %d: %d", Integer.valueOf(this.maxFrameSize), Integer.valueOf(i2));
            } else if ((Integer.MIN_VALUE & i) != 0) {
                throw Http20Draft16.illegalArgument("reserved bit set: %s", Integer.valueOf(i));
            } else {
                Http20Draft16.writeMedium(this.sink, i2);
