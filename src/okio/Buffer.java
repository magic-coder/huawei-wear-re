...
public final class Buffer implements Cloneable, BufferedSink, BufferedSource {
    private static final byte[] DIGITS = new byte[]{TagName.APK_SIZE, TagName.NOTICE_ID, TagName.NOTICE_TITLE, TagName.ACTIVITY_CODE_LIST, TagName.NOTICE_BODY, (byte) 53, TagName.NOTICE_START_TIME, TagName.NOTICE_END_TIME, ScriptToolsConst.TagName.TagSerial, ScriptToolsConst.TagName.TagApdu, TagName.MAIN_ORDER_LIST, TagName.OPERATE_TIMING, TagName.PAY_ORDER, TagName.PAY_ORDER_LIST, TagName.ORDER_TYPE, (byte) 102};
    static final int REPLACEMENT_CHARACTER = 65533;
    Segment head;
    long size;

    class C27561 extends OutputStream {
        C27561() {
        }
    ...
    class C27572 extends InputStream {
        C27572() {
        }
    ...
    public long readLong() {
        ...
        int i2 = segment.limit;
        if (i2 - i < 8) {
            return ((((long) readInt()) & MessageHead.SERIAL_MAK) << 32) | (((long) readInt()) & MessageHead.SERIAL_MAK);
        }
        byte[] bArr = segment.data;
        int i3 = i + 1;
    ...
    public long readHexadecimalUnsignedLong() {
        ...
        do {
            ...
            while (i4 < i3) {
                byte b = bArr[i4];
                if (b >= TagName.APK_SIZE && b <= ScriptToolsConst.TagName.TagApdu) {
                    i2 = b - 48;
                } else if (b >= TagName.MAIN_ORDER_LIST && b <= (byte) 102) {
                    i2 = (b - 97) + 10;
                } else if (b < TagName.TERMINAL_BACK_CONTENT || b > TagName.TERMINAL_BASEBAND_VERSION) {
                    if (i != 0) {
                        obj = 1;
                        if (i4 != i3) {
                            this.head = segment.pop();
    ...
    String readUtf8Line(long j) throws EOFException {
        if (j <= 0 || getByte(j - 1) != TagName.PAY_CHANNEL) {
    ...
    public Buffer writeDecimalLong(long j) {
        ...
        if (obj != null) {
            bArr[i2 - 1] = TagName.APK_UPDATE_FLAG;
        }
        writableSegment.limit += i;
            ...
            while (j > 0) {
                ...
                if (j < ((long) (buffer.head.limit - buffer.head.pos))) {
                    ...
                    if (segment != null && segment.owner) {
                        if ((((long) segment.limit) + j) - ((long) (segment.shared ? 0 : segment.pos)) <= PlaybackStateCompat.ACTION_PLAY_FROM_SEARCH) {
